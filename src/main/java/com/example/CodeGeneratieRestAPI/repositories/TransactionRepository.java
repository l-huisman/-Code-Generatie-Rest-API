...
//            "or (:amountRelation = '>' and t.amount > :amount)\n" +
//            "or (:amountRelation = '=' and t.amount = :amount))")
    @Query("SELECT t FROM Transaction t LEFT JOIN t.toAccount LEFT JOIN t.fromAccount WHERE t.createdAt <= :endDate AND t.createdAt >= :startDate AND (:amount is null OR (:amountRelation = '<' AND t.amount < :amount) OR (:amountRelation = '>' AND t.amount > :amount) OR (:amountRelation = '=' AND t.amount = :amount)) AND (:iban = '' OR :iban is null OR (t.fromAccount IS NOT null AND t.fromAccount.iban = :iban ) OR (t.toAccount IS NOT null and t.toAccount.iban = :iban))")
