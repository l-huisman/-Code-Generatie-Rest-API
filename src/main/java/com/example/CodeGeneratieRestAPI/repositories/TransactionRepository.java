...
public interface TransactionRepository extends CrudRepository<Transaction, Long> {
    ...
    @Query("SELECT t FROM Transaction t LEFT JOIN t.toAccount LEFT JOIN t.fromAccount WHERE t.createdAt <= :endDate AND t.createdAt >= :startDate OR ((t.fromAccount IS NOT null AND t.fromAccount.iban = :iban ) OR (t.toAccount IS NOT null and t.toAccount.iban = :iban)) OR (:balanceRelation is null\n" +
            "or (:balanceRelation = '<' and t.amount < :tnxAmt)\n" +
            "or (:balanceRelation = '>' and t.amount > :tnxAmt)\n" +
            "or (:balanceRelation = '=' and t.amount = :tnxAmt))")
    Page<Transaction> findAll(Date endDate, Date startDate, String iban, String balanceRelation, Float balance, Pageable pageable);
